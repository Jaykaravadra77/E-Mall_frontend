...
function OrderDetail() {
    ...
    const handleStatusChange = (e, orderId) => {
        updateOrderStatus(checkAuth.token, orderId, e.target.value).then(
            data => {
                if (data.error) {
                        ...
                        {order && order.length === 1 ?
                            ...
                            <>
                                <ul className="list-group">
                                    ...
                                    <li className="list-group-item">Transaction_id:{order[0].transaction_id}</li>
                                    <li className="list-group-item">Amount:{order[0].amount}</li>
                                    <li className="list-group-item">Order On:{moment(order[0].createdAt).fromNow()}</li>
                                    <li className="list-group-item">  Delivery address: {order[0].address}</li>
                                </ul>
                                <h5 className="mt-4 text-bg-secondary">Products associated with order </h5>
